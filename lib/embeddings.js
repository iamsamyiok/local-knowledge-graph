...
async function build(progress) {
  ...
  for (let i = 0; i < pending.length; i += BATCH) {
    ...
    const vectors = await embed(texts, s);
    chunk.forEach((e, k) => db.upsertEmbedding(e.id, texts[k], vectors[k]));
    if (progress) progress(done + Math.min(i + BATCH, pending.length), ents.length);
  }
  return { indexed: db.countEmbeddings(), total: ents.length };
}
...
async function search(query, topK) {
  ...
  let sem = [];
  if (s.api_key && db.countEmbeddings() > 0) {
    const [qvec] = await embed([query], s);
    sem = db.allEmbeddings()
  ...
  return { results, mode: s.api_key && db.countEmbeddings() > 0 ? 'hybrid' : 'keyword_only' };
}
