...
const DB_PATH = path.join(DATA_DIR, 'kg.db');
...
function addEntityImage(entityId, item) {
  ...
  const info = db.prepare('INSERT INTO entity_images (entity_id, filename, stored_path, caption) VALUES (?, ?, ?, ?)')
    .run(entityId, item.filename.trim().slice(0, 200), item.stored_path.trim(), caption);
...
function countEmbeddings() {
  return db.prepare('SELECT COUNT(*) AS c FROM entity_embeddings').get().c;
}

function getEmbedding(entityId) {
  return db.prepare('SELECT * FROM entity_embeddings WHERE entity_id = ?').get(entityId) || null;
}

function allEmbeddings() {
  return db.prepare('SELECT entity_id, text, vector FROM entity_embeddings').all();
}

function upsertEmbedding(entityId, text, vec) {
...
module.exports = {
  ...
  addEntityImage, getEntityImage, listEntityImages, deleteEntityImage, imageCounts,
  egoSubgraph,
  countEmbeddings, getEmbedding, allEmbeddings, upsertEmbedding, miniCypher,
