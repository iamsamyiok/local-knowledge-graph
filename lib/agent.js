...
// ---------- OpenCode 可执行文件解析 ----------
// Windows 下 npm 全局安装产生 .cmd shim，spawn('opencode') 无法直接执行（.cmd 需要 shell，而 prompt
// 是 argv 传参存在注入面），因此优先解析包内真实的 opencode.exe 绝对路径；Unix 直接用 PATH 上的 opencode。
let opencodeCmdCache; // undefined=未探测 ''=探测失败
function resolveOpencodeCommand(force = false) {
  if (!force && opencodeCmdCache !== undefined) return opencodeCmdCache;
  const candidates = [];
  ...
  for (const c of candidates) {
    try {
      execFileSync(c, ['--version'], { timeout: 15000, stdio: ['ignore', 'pipe', 'pipe'] });
      opencodeCmdCache = c;
      return c;
    } catch (_) { /* 尝试下一个候选 */ }
  }
...
- 需要按关键词或关系模式检索时，运行只读命令：
  node tools/search.js keyword "<关键词>" [数量]（名称/属性关键词检索，附每个实体的关系列表）
  node tools/search.js cypher "MATCH (a)-[r:互动]->(b) WHERE a.name contains 词 RETURN a,r,b LIMIT 20"（按关系类型/名称与实体条件检索）
  建议先用 keyword 找到实体名，再用 cypher 按关系类型展开。
- 需要挖掘隐性/间接关系（如"大雁塔和长安有什么联系"）时，运行只读命令：node -e "const{computeInferred}=require('./lib/inference');const db=require('./lib/db');const g=db.getGraph();console.log(JSON.stringify(computeInferred(g),null,1))"
  该推理引擎按OWL传递/对称/逆规则推导库中未显式存储的关系（含推导依据）。也可访问 http://localhost:3000/api/inference 获取同样结果。回答时请区分"库中显式关系"与"推理得出的隐性关系"。
...
function runOpenCodeRaw(preArgs, prompt, files, timeoutMs, envExtra) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      if (!opencodeCmd) return resolve({ ok: false, error: '未检测到 OpenCode，可在 AI 面板点击一键安装或执行 npm install -g opencode-ai' });
      child = spawn(opencodeCmd, args, { cwd: APP_ROOT, env: { ...process.env, ...(envExtra || {}) }, stdio: ['ignore', 'pipe', 'pipe'] });
    } catch (e) {
      return resolve({ ok: false, error: '无法启动OpenCode进程: ' + e.message });
    }

    const timer = setTimeout(() => {
      if (!done) { done = true; try { child.kill('SIGKILL'); } catch (_) {} resolve({ ok: false, error: `OpenCode执行超时(${timeoutMs / 1000}s)，已终止` }); }
    }, timeoutMs);
    ...
    child.on('close', (code) => {
      ...
      resolve({ ok: true, texts: ev.texts || stdout.trim(), sessionId: ev.sessionId || null });
    });
  });
}
...
function attempt(sessionId, instruction, files) {
  ...
  return runOpenCodeRaw(preArgs, buildPrompt(instruction), files, TIMEOUT_MS).then((r) => {
    if (!r.ok) return r;
    if (r.sessionId) writeSession(r.sessionId);
    const replyText = r.texts;
    const parsed = parseOps(replyText);
    return { ok: true, reply: parsed.reply || replyText, ops: parsed.ops, parse_error: parsed.parse_error || null, session: readSession() };
  });
}
...
module.exports = { buildPrompt, runAgent, runPlain, parseOps, parseEventStream, resolveOpencodeCommand };
